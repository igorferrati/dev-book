...
// CriarPublicacao cria uma nova publicacão
...
// BuscarPublicacoes traz as publicacoes do feed do usuário
...
// AtualizarPublicacao altera os dados de uma publicacão
func AtualizarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	repositorio := repositorios.NovoRepositorioDePublicacoes(db)
	publicacaoSalvaNoBanco, erro := repositorio.BuscaPorID(uint64(publicacaoID))
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
...
// DeletarPublicacao dele te uma publicacão
...
// BuscarPublicacoesPorUsuario traz todos publicacões de um usuário específico
...
// CurtirPublicao adiciona uma curtida na publicacão
func CurtirPublicao(w http.ResponseWriter, r *http.Request) {
	parametros := mux.Vars(r)
	publicacaoID, erro := strconv.ParseUint(parametros["publicacaoId"], 10, 64)
	if erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
...
// DescurtirPublicao subtrai uma curtida na publicacão
func DescurtirPublicao(w http.ResponseWriter, r *http.Request) {
	parametros := mux.Vars(r)
	publicacaoID, erro := strconv.ParseUint(parametros["publicacaoId"], 10, 64)
	if erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
